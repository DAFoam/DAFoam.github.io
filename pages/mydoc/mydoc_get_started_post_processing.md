...
In this case, we use the SLSQP optimizer that prints objective function value from the 1st iteration. To find the objective for the 0th iteration (baseline design), you need to go back to `logOpt.txt` (see more detailed explantion of this file from [this page](mydoc_get_started_runscript.html)):
...
{% include note.html content="The `Decomposed Case` type tells Paraview to load data from `processor*` folder since we ran this case in parallel using 4 CPU cores. If one runs a serial run with one CPU core, select **Reconstructed Case** for `Case Type` instead." %}
...
Next, scroll down at the left panel and check `Camera Paralle Projection`.

{% include note.html content="The `Camera Paralle Projection` option is preferable for zoom-in visualization." %}

<img src="{{ site.url }}{{ site.baseurl }}/images/tutorials/Paraview_Parallel_Projection.png" width="300" />

Fig. 4. Check Camera Paralle Projection

|

|

Now, click `Apply` at the left panel to load the flow fields. By default, the pressure field (`p`) will be load, but you can choose other flow variables to load at the top panel. Also, the `Surface` prepresentation will be used by default, you can change it to `Surface With Edges` to visualize the mesh.
