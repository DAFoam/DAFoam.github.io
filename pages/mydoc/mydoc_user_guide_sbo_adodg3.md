---
title: ADODG3 wing - 3D aerodynamic surrogate based shape optimization
keywords: user guide
summary: Surrogate based optimization for a 3D wing case to minimize drag
sidebar: mydoc_sidebar
permalink: user-guide-SBO-adodg3.html
folder: mydoc
---
...
In previous surrogate based optimizations, we applied the method to a 2D case, the NACA0012 optimization. In this guide we will expand this to cover a 3D optimization case.
...
Design variables: 20 FFD points, twist, and angle of attack (total: 22).
...
The surrogate based optimization capability in DAFoam is designed in such a way that any DAFoam opitmization case can be converted to a surrogate based optimization case by adding one additional python file to configure the surrogate optimization. For this variation of the NACA0012, this file is `runScript_SBO.py` which can be found in the [tutorials](https://github.com/DAFoam/tutorials/archive/main.tar.gz) under the `NACA0012_Airfoil/incompressible` case folder.
...
2. 0.5 $\leq$ airfoil thickness $\leq$ 3

3. 1 $\leq$ airfoil volume
